...
// ============================================================
// Zero dependencies · UMD · MIT License
...
;(function (root, factory) {
  ...
  function autoAdapt() {
    ...
    if (typeof uni !== 'undefined') {
      ...
      adapter('net', {
        type: function () {
          var type = 'unknown'
          try { wx.getNetworkType({ success: function(res) { type = res.networkType || 'unknown' } }) } catch(e) {}
          return type
        },
        watch: function(cb) {
      ...
      var _mem = {}
      adapter('storage', { get: function(k) { return _mem[k]||null }, set: function(k,v) { _mem[k]=v }, del: function(k) { delete _mem[k] } })
    }
    else {
      var _m2 = {}
  ...
  function init(cfg) {
    ...
    if (_cfg.captureGlobal) {
      try {
        if (typeof uni !== 'undefined' && uni.onError) { uni.onError(function(e) { _emit(LEVEL.FATAL, CAT.CRASH, 'JS Crash', String(e), '') }) }
        else if (typeof window !== 'undefined') { window.addEventListener('error', function(e) { _emit(LEVEL.FATAL, CAT.CRASH, 'JS Crash', e.message||String(e.error), (e.error&&e.error.stack)||'') }) }
      } catch(e) {}
    }
    if (_cfg.capturePromise) {
      try {
        if (typeof uni !== 'undefined' && uni.onUnhandledRejection) { uni.onUnhandledRejection(function(r) { var m=r&&r.reason; _emit(LEVEL.ERROR, CAT.CRASH, 'Unhandled Promise', m instanceof Error?m.message:String(m||''), m instanceof Error?m.stack:'') }) }
        else if (typeof window !== 'undefined') { window.addEventListener('unhandledrejection', function(e) { _emit(LEVEL.ERROR, CAT.CRASH, 'Unhandled Promise', String(e.reason||''), (e.reason&&e.reason.stack)||'') }) }
      } catch(e) {}
    }
    if (_adapt.net && _adapt.net.watch) { _adapt.net.watch(function(type) { _dev.netType = type; _crumb('net:change', type); _emit(LEVEL.INFO, CAT.NETWORK, 'net:change', type) }) }
  ...
  function _emit(level, category, tag, message, stack, extra) {
    ...
    var log = {
      id:     _seq, time: new Date(now).toISOString(), ts: now, level: level,
      levelLabel:  LEVEL_LABEL[level] || '?', levelName: LEVEL_NAME[level] || '?',
      cat:    Object.values(CAT).indexOf(category)>=0 ? category : CAT.APP,
      tag:    String(tag||''), msg: safeMsg, stack: String(stack||''),
    ...
    _logs.push(log); while (_logs.length > _cfg.maxLogs) _logs.shift()
    ...
    _notify(log); return log
  }

  function _notify(log) { if (!_watch.length) return; setTimeout(function() { _watch.forEach(function(cb) { try { cb(log) } catch(e) {} }) }, 0) }
  ...
  var net = {
    req: function(method, url, status, dur, size) {
      ...
      return _emit(lvl, CAT.NETWORK, 'net:req', method + ' ' + _sanitize(String(url).slice(-80)), '', { method:method, url:_sanitize(url), status:status, duration:dur, size:size||0 })
    },
    err: function(url, err) { return _emit(LEVEL.ERROR, CAT.NETWORK, 'net:err', String(err), '', { url:_sanitize(url) }) },
    slow: function(url, ms) { return _emit(LEVEL.WARN, CAT.NETWORK, 'net:slow', ms+'ms '+_sanitize(String(url).slice(-80)), '', { url:_sanitize(url), duration:ms }) },
    timeout: function(url, ms) { return _emit(LEVEL.ERROR, CAT.NETWORK, 'net:timeout', (ms||'?')+'ms', '', { url:_sanitize(url) }) }
  }
  ...
  var wxWrap = {
    req: function(opts) {
      ...
      opts.success = function(res) { var dur = Date.now() - start, size = 0; try { size = JSON.stringify(res.data||'').length } catch(e) {}; net.req(opts.method||'GET', opts.url, res.statusCode, dur, size); if (typeof origSuccess === 'function') origSuccess(res); if (typeof origComplete === 'function') origComplete(res) }
      opts.fail = function(err) { net.err(opts.url, err.errMsg||'wx.request failed'); if (typeof origFail === 'function') origFail(err); if (typeof origComplete === 'function') origComplete(err) }
      wx.request(opts)
    },
    get: function(url, data, success, fail) { wxWrap.req({ url: url, method: 'GET', data: data, success: success, fail: fail }) },
    post: function(url, data, success, fail) { wxWrap.req({ url: url, method: 'POST', data: data, success: success, fail: fail }) }
  }

  var _netFetchRef = null, _netXHROpenRef = null, _netXHRSendRef = null
  ...
  function _patchNetwork() {
    ...
    if (window.fetch && !window.fetch.__br_patched) {
      ...
      window.fetch = function(input, init) {
        var url = '', method = ((init && init.method) || 'GET').toUpperCase()
        try { if (typeof input === 'string') { url = input } else if (input && typeof input.url === 'string') { url = input.url; method = (input.method || method).toUpperCase() } else { url = String(input||'') } } catch(e) { url = String(input||'') }
        var start = Date.now()
        return _netFetchRef.call(this, input, init).then(function(resp) { var dur = Date.now() - start, size = 0; try { var cl = resp.headers.get('content-length'); if (cl) size = parseInt(cl)||0 } catch(e) {}; net.req(method, _sanitize(url), resp.status, dur, size); return resp }).catch(function(err) { net.err(_sanitize(url), err.message || String(err)); throw err })
      }
      window.fetch.__br_patched = true
    }
    if (typeof XMLHttpRequest !== 'undefined' && !XMLHttpRequest.__br_patched) {
      _netXHROpenRef = XMLHttpRequest.prototype.open; _netXHRSendRef = XMLHttpRequest.prototype.send
      XMLHttpRequest.prototype.open = function(method, url) { this.__br_method = method; this.__br_url = url; return _netXHROpenRef.apply(this, arguments) }
      XMLHttpRequest.prototype.send = function() { var self = this; self.__br_start = Date.now(); self.addEventListener('loadend', function() { var dur = Date.now() - (self.__br_start || Date.now()), size = 0; try { var cl = self.getResponseHeader('content-length'); if (cl) size = parseInt(cl)||0 } catch(e) {}; try { net.req(self.__br_method||'?', _sanitize(self.__br_url||''), self.status, dur, size) } catch(e) {} }); self.addEventListener('error', function() { try { net.err(_sanitize(self.__br_url||''), 'XHR Error') } catch(e) {} }); self.addEventListener('timeout', function() { try { net.timeout(_sanitize(self.__br_url||''), Date.now() - (self.__br_start||Date.now())) } catch(e) {} }); return _netXHRSendRef.apply(this, arguments) }
      XMLHttpRequest.__br_patched = true
    }
  }

  function _unpatchNetwork() { if (typeof window === 'undefined') return; if (_netFetchRef) { window.fetch = _netFetchRef; _netFetchRef = null }; if (_netXHROpenRef) { XMLHttpRequest.prototype.open = _netXHROpenRef; _netXHROpenRef = null }; if (_netXHRSendRef) { XMLHttpRequest.prototype.send = _netXHRSendRef; _netXHRSendRef = null } }

  var perf = {
    start: function(name) { _trace[name] = Date.now() },
    end: function(name, threshold) { var t = _trace[name]; if (!t) return -1; delete _trace[name]; var ms = Date.now() - t; var lvl = ms > (threshold||3000) ? LEVEL.WARN : LEVEL.INFO; _emit(lvl, CAT.PERF, 'perf:'+name, ms+'ms', '', { duration:ms, threshold:threshold }); return ms },
  ...
  function watch(cb) { if (typeof cb !== 'function') return function(){}; _watch.push(cb); return function() { _watch = _watch.filter(function(w) { return w !== cb }) } }

  function query(f) {
    f = f || {}; var r = _logs.slice()
  ...
  function stats() { init(); return { total: _logs.length, errors: errCount(), warnings: wrnCount(), fatals: _stats.fatals, sessionMs: Date.now() - _stats.start, byCat: Object.assign({}, _stats.byCat), byPage: Object.assign({}, _stats.byPage), device: Object.assign({}, _dev) } }

  function exportLogs(format, f) {
    var logs = query(f), dev = _dev
    if (format === 'csv') { var c = 'id,time,level,category,tag,message,page\n'; logs.forEach(function(l) { c += [l.id, l.time, l.levelName, l.cat, l.tag, '"'+(l.msg||'').replace(/"/g,'""')+'"', l.page].join(',') + '\n' }); return c }
    if (format === 'text') {
      var t = 'BRS Log Export\n'; t += new Array(61).join('=') + '\n'; t += 'Device:  ' + [dev.brand, dev.model].filter(Boolean).join(' ') + '\n'; t += 'System:  ' + [dev.system, dev.osVer].filter(Boolean).join(' ') + '\n'; t += 'App:     ' + [dev.appName, 'v'+dev.appVer].filter(Boolean).join(' ') + '\n'; t += 'Export:  ' + new Date().toISOString() + '\n'; t += 'Logs:    ' + logs.length + ' | Errors: ' + errCount() + ' | Warnings: ' + wrnCount() + '\n'; t += new Array(61).join('=') + '\n\n'
      if (!logs.length) { t += 'No logs.\n'; return t }
      logs.forEach(function(l) { var icon = l.level >= LEVEL.ERROR ? '[E]' : l.level >= LEVEL.WARN ? '[W]' : l.level >= LEVEL.INFO ? '[I]' : '[D]'; t += '#' + l.id + ' ' + icon + ' ' + l.time.slice(11,23) + ' ' + l.cat + ' ' + l.tag + '\n  page: ' + l.page + '\n'; if (l.msg) t += '  ' + l.msg + '\n'; if (l.stack) t += '  stack: ' + l.stack.split('\n').slice(0,3).join('\n  > ') + '\n'; if (l.extra) { try { t += '  data: ' + JSON.stringify(l.extra) + '\n' } catch(e) {} }; t += '\n' })
      return t
    }
    return JSON.stringify({ exportedAt: new Date().toISOString(), device: dev, stats: stats(), filter: f, logs: logs }, null, 2)
  }

  function copyLogs(f) { var text = exportLogs('text', f); return new Promise(function(resolve) { if (typeof wx !== 'undefined' && wx.setClipboardData) { wx.setClipboardData({ data: text, success: function(){resolve(true)}, fail: function(){resolve(false)} }) } else if (typeof uni !== 'undefined') { uni.setClipboardData({ data: text, success: function(){resolve(true)}, fail: function(){resolve(false)} }) } else if (typeof navigator !== 'undefined' && navigator.clipboard) { navigator.clipboard.writeText(text).then(function(){resolve(true)}).catch(function(){resolve(false)}) } else { resolve(false) } }) }

  function _persist() { if (!_cfg.persist || !_adapt.storage) return; try { _adapt.storage.set(STORAGE_KEY, _logs.slice(-_cfg.maxLogs)); _adapt.storage.set(STATS_KEY, { errors:_stats.errors, warns:_stats.warns, fatals:_stats.fatals, byCat:_stats.byCat, byPage:_stats.byPage }) } catch(e) {} }

  function clear() { _logs=[]; _seq=0; _stats.errors=0; _stats.warns=0; _stats.fatals=0; _stats.byCat={}; _stats.byPage={}; try { if(_adapt.storage) { _adapt.storage.del(STORAGE_KEY); _adapt.storage.del(STATS_KEY) } } catch(e) {} }
  ...
  return {
    ...
    watch: watch,
    crumb: function(t,m) { _crumb(t,m) }, crumbs: function() { return _crumbs.slice() }, clearCrumbs: function() { _crumbs=[] },
    snapshot: function(d) { _snapshot = d }, getSnapshot: function() { return _snapshot }, clearSnapshot: function() { _snapshot = null },
    sanitizer: { addRule: function(r) { if (r instanceof RegExp) _sanitizerRules.push(r) }, rules: function() { return _sanitizerRules.slice() }, sanitize: _sanitize },
